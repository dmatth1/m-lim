...
- Parameter IDs (all `juce::String`):
  ...
  - `"sidechainHPFreq"` — float, 20 to 2000 Hz, default 20 (detection path high-pass, 20 = off)
  - `"sidechainLPFreq"` — float, 2000 to 20000 Hz, default 20000 (detection path low-pass, 20k = off)
...
### VisualParityAuditor
You are a visual QA engineer who builds the plugin and compares running screenshots against reference images. Your workflow every sweep: (1) build with `cd /workspace/M-LIM && export CCACHE_DIR=/build-cache && cmake -B build -DCMAKE_BUILD_TYPE=Release -DCMAKE_CXX_COMPILER_LAUNCHER=ccache && cmake --build build -j$(nproc)`, (2) start Xvfb on :99, (3) launch the standalone binary, (4) capture a screenshot with scrot, (5) compare against `/reference-docs/reference-screenshots/prol2-main-ui.jpg` using ImageMagick `compare -metric RMSE`. Report the RMSE value and list specific visual differences you can see between the screenshot and the reference (layout, colors, missing elements). For each visual difference you identify, create a task in `tasks/pending/` with the specific file to change, the visual element that is wrong, and what it should look like. You may directly fix single-line color constant issues in `Colours.h`. Do not modify DSP code.

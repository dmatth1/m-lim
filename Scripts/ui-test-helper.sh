...
# Find the Standalone binary, checking several possible locations.
_find_standalone() {
  local candidates=(
...
start_app() {
  ...
  if [ -f "/tmp/.X${DISPLAY_NUM}-lock" ]; then
    local old_pid
    old_pid=$(cat "/tmp/.X${DISPLAY_NUM}-lock" 2>/dev/null | tr -d ' ')
  ...
  local binary
  binary=$(_find_standalone) || return 1

  echo "Launching Standalone: $binary"
  "$binary" &
  APP_PID=$!
  sleep 1
  ...
  while [ $waited -lt 10 ]; do
    if xdotool search --onlyvisible --name "." 2>/dev/null | head -1 | grep -q .; then
      echo "Window detected after ${waited}s"
      return 0
    fi
    sleep 1
    waited=$((waited + 1))
  ...
  echo "WARNING: No window detected after 10s (app may still be starting)" >&2
  return 0
}
...
# wait_for_paint [max_wait_ms]
wait_for_paint() {
  local max_wait="${1:-2000}"
  ...
  while true; do
    ...
    rm -f "$tmp"
    if [ "$(echo "$mean > 1000" | bc)" = "1" ]; then
      return 0
    fi
    local elapsed=$(( $(date +%s%3N) - start_ms ))
    if [ $elapsed -gt $max_wait ]; then
      echo "WARNING: paint timeout after ${max_wait}ms" >&2
      return 1
    fi
    sleep 0.1
  done
...
# screenshot_safe [filename] [max_wait_ms]
screenshot_safe() {
  local filename="${1:-screenshot.png}"
  local max_wait="${2:-2000}"
  wait_for_paint "$max_wait"
  screenshot "$filename"
}

# compare_to_reference <reference_image> <captured_image> [rmse_threshold]
compare_to_reference() {
  ...
  if [ ! -f "$captured" ]; then
    echo "ERROR: captured image not found: $captured" >&2
    return 1
  fi

  local diff_out="${captured%.*}_diff.png"
  local rmse
  rmse=$(compare -metric RMSE "$ref" "$captured" "$diff_out" 2>&1 | grep -Eo '[0-9]+\.[0-9]+' | head -1)

  if [ -z "$rmse" ]; then
    rmse=$(compare -metric RMSE "$ref" "$captured" "$diff_out" 2>&1 | grep -Eo '^[0-9]+' | head -1)
    rmse="${rmse:-1}"
  fi

  echo "RMSE: $rmse (threshold: $threshold) — ref: $(basename "$ref")"

  if awk "BEGIN {exit ($rmse <= $threshold) ? 0 : 1}"; then
    echo "PASS"
    return 0
  else
    echo "FAIL — diff written to: $diff_out"
    return 1
  fi
}

# screenshot_all_states <prefix>
screenshot_all_states() {
  local prefix="${1:-parity}"

  echo "Capturing: default state"
  wait_for_render 500
  screenshot "${prefix}-default.png"

  echo "Capturing: with gain reduction active"
  # Additional states can be added as UI develops
}
